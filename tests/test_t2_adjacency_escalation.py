...
def _secured_forward_region() -> dict[str, Any]:
    return {
        "key": "region:forward",
        "state": "secured",
        "position": [120, 3, 120],
        ...
        "connected": True,
        "reclaimAnchor": True,
        "memberKeys": ["Mass:120000:120000"],
    }


def test_idle_t2_builder_patrols_secured_forward_region_after_power_completes() -> None:
    harness = make_harness()
    harness.lua.execute("Policy.Decide = function() return {} end")
    plan = _macro_plan(lane="reclaim", grant=False)
    plan["regions"] = [_secured_forward_region()]
    _set_director_result(harness, "macroPlan", plan)
    harness.brain.units = harness.lua.table_from(
        [
            harness.unit(entityId=20, blueprintId="ueb1201", position=[40, 2, 40]),
            harness.unit(entityId=21, blueprintId="uel0208", position=[42, 2, 40]),
        ]
    )
    ...
    assert harness.controller.reclaimPatrolAssignments["21:1"] is True
    patrols = [plain(call.position) for call in harness.calls.patrol.values()]
    assert [(position[0], position[2]) for position in patrols] == [
        (144, 120),
        (120, 144),
        (96, 120),
        (120, 96),
...
@pytest.mark.parametrize(
    ("power", "state", "anchor", "expected"),
    [
        (False, "secured", True, 0),
        (True, "establishing", True, 0),
        (True, "contested", True, 0),
        (True, "suspended", True, 0),
        (True, "secured", False, 0),
        (True, "secured", True, 4),
    ],
)
def test_forward_reclaim_patrol_requires_completed_power_and_safe_region(
    power: bool, state: str, anchor: bool, expected: int
) -> None:
    harness = make_harness()
    harness.lua.execute("Policy.Decide = function() return {} end")
    region = _secured_forward_region()
    region["state"] = state
    region["reclaimAnchor"] = anchor
    plan = _macro_plan(lane="reclaim", grant=False)
    plan["regions"] = [region]
    _set_director_result(harness, "macroPlan", plan)
    units = [harness.unit(entityId=21, blueprintId="uel0208", position=[42, 2, 40])]
    if power:
        units.insert(0, harness.unit(entityId=20, blueprintId="ueb1201"))
    harness.brain.units = harness.lua.table_from(units)

    harness.lua.globals().Controller.Step(harness.controller)

    assert len(harness.calls.patrol) == expected


def test_forward_reclaim_patrol_is_persistent_and_not_reissued() -> None:
    harness = make_harness()
    harness.lua.execute("Policy.Decide = function() return {} end")
    plan = _macro_plan(lane="reclaim", grant=False)
    plan["regions"] = [_secured_forward_region()]
    ...
    harness.brain.units = harness.lua.table_from(
        [
            harness.unit(entityId=20, blueprintId="ueb1201"),
            harness.unit(entityId=21, blueprintId="uel0208"),
        ]
    )
...
@pytest.mark.parametrize("tamper", ["region", "waypoint", "captured"])
def test_forward_reclaim_patrol_executor_fails_closed(tamper: str) -> None:
    harness = make_harness()
    engineer = harness.unit(entityId=21, blueprintId="uel0208")
    if tamper == "captured":
        engineer.options.army = 2
    harness.brain.units = harness.lua.table_from(
        [harness.unit(entityId=20, blueprintId="ueb1201"), engineer]
    )
    region = _secured_forward_region()
    harness.controller.macroPlan = lua_value(
        harness.lua, {"regions": [] if tamper == "region" else [region]}
    )
    waypoints = [
        [144, 3, 120],
        [120, 3, 144],
    ...
    assert harness.controller.reclaimPatrolAssignments["21:1"] is None
